...
@IF /I [%1]==[] (
    @echo:By Gomiero ^(c^) 2025-2025 - for private use, only by THE OWNER
    @echo:
    @echo Converts a string to urlencode and, optionally, send it to clipboard
    @echo:    
    @echo s2url "[string]"
    @echo s2url -c "[string]"
    @echo:
...
@IF /I [%1] EQU [-c] (
    @IF "%~2" == "" (
        ...
        @python %~dp0\s2url.py %~2 | clip
        goto :END
    )
) ELSE (
    @python %~dp0\s2url.py %~1
)
